...
class ClientLoginAuthentication(Choreography):
    ...
    def __init__(self, temboo_session):
        Choreography.__init__(self, temboo_session, '/Library/Google/Spreadsheets/ClientLoginAuthentication')
...
class ClientLoginAuthenticationInputSet(InputSet):
        ...
        """
        Set the value of the Service input for this choreography. ((required, string) The service that you are requesting an authorization token for. Defaults to 'wise' for Spreadsheets API.)
        """
...
class ClientLoginAuthenticationResultSet(ResultSet):
        """
        Retrieve the value for the "AuthorizationKey" output from this choreography execution. ((string) The auth token parsed from the Google login response)
        """
