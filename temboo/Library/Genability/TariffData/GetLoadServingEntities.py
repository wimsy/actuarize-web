...
class GetLoadServingEntities(Choreography):
    ...
    """
    Create a new instance of the GetLoadServingEntities Choreography. A TembooSession object, containing a valid
...
class GetLoadServingEntitiesInputSet(InputSet):
        """
        Set the value of the AccountID input for this choreography. ((optional, string) The unique ID of the Account to find LSEs for. When passed in, the search will look for a territoryId on the Account and use that to find all LSEs that provide service within that territory.)
        """
...
"""
A ResultSet with methods tailored to the values returned by the GetLoadServingEntities choreography.
