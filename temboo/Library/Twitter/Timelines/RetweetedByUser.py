...
class RetweetedByUser(Choreography):
    ...
    """
    Create a new instance of the RetweetedByUser Choreography. A TembooSession object, containing a valid
    set of Temboo credentials, must be supplied.
    """
    def __init__(self, temboo_session):
        Choreography.__init__(self, temboo_session, '/Library/Twitter/Timelines/RetweetedByUser')
    ...
    def _make_result_set(self, result, path):
        return RetweetedByUserResultSet(result, path)

    def _make_execution(self, session, exec_id, path):
        return RetweetedByUserChoreographyExecution(session, exec_id, path)

"""
An InputSet with methods appropriate for specifying the inputs to the RetweetedByUser
choreography. The InputSet object is used to specify input parameters when executing this choreo.
"""
class RetweetedByUserInputSet(InputSet):
        """
        ...
        """
        Set the value of the IncludeEntities input for this choreography. ((optional, boolean) An advanced option for returning more verbose metadata. When set to either true or 1, each tweet will include a node called "entities".)
        """
        def set_IncludeEntities(self, value):
            InputSet._set_input(self, 'IncludeEntities', value)

        """
        Set the value of the IncludeRetweets input for this choreography. ((optional, boolean) When set to either true or 1, the timeline will contain native retweets (if they exist) in addition to the standard stream of tweets.)
        """
        ...
        """
        Set the value of the ScreenName input for this choreography. ((required, string) The screen name of the user for whom to return results for.)
        """
...
"""
A ResultSet with methods tailored to the values returned by the RetweetedByUser choreography.
The ResultSet object is used to retrieve the results of a choreography execution.
"""
class RetweetedByUserResultSet(ResultSet):
        """
...
class RetweetedByUserChoreographyExecution(ChoreographyExecution):
    
    def _make_result_set(self, response, path):
        return RetweetedByUserResultSet(response, path)
