...
class SearchPlaylistsInputSet(InputSet):
        ...
        """
        Set the value of the Playlist input for this choreography. ((required, string) The name of a playlist to search for. Separate multiple playlist names with commas (but no spaces).)
        """
        def set_Playlist(self, value):
            InputSet._set_input(self, 'Playlist', value)

        """
        Set the value of the ResponseFormat input for this choreography. ((required, string) The format of the response from YouTube. Accepts atom, rss, json, json-in-script, and jsonc. Defaults to atom.)
        """
